                        ...
                        <div class="table-responsive mb-4 mt-4">
                            <table id="html5-extension" class="table table-hover non-hover" style="width:100%">
                                ...
                                <tbody>
                                    ...
                                    @php
                                    $obj =
                                    DB::table('records')->join('record_products','record_products.record_id','records.id')->where('invoice_number',$match->sale_invoice)->first();
                                    $purchase_obj =
                                    DB::table('purchase_records')->join('purchase_record_products','purchase_record_products.purchase_record_id','purchase_records.id')->where('invoice_number',$match->purchase_invoice)->first();
                                    @endphp
                                    <tr>
                                        <td>{{ $sno++ }}</td>
                                        ...
                                        @php
                                        $obj =
                                        DB::table('records')->join('record_products','record_products.record_id','records.id')->where('invoice_number',
                                        $match->sale_invoice)->where('product_id',$product->id)->first();
                                        $purchase_obj =
                                        DB::table('purchase_records')->join('purchase_record_products','purchase_record_products.purchase_record_id','purchase_records.id')->where('invoice_number',$match->purchase_invoice)->where('product_id',$product->id)->first();
                                        ...
                                        $purchase_obj =
                                        DB::table('purchase_records')->where('invoice_number',$match->purchase_invoice)->first();

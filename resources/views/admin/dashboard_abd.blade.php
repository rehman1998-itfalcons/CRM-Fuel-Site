...
@section('contents')
    ...
    $last_day_1 = Carbon::now()
        ->subdays(1)
        ->format('d-m-Y');
    $last_day_2 = Carbon::now()
        ->subdays(2)
        ->format('d-m-Y');
    $last_day_3 = Carbon::now()
        ->subdays(3)
        ->format('d-m-Y');
    $last_day_4 = Carbon::now()
        ->subdays(4)
        ->format('d-m-Y');
    $last_day_5 = Carbon::now()
        ->subdays(5)
        ->format('d-m-Y');
    $last_day_6 = Carbon::now()
        ->subdays(6)
        ->format('d-m-Y');
    $last_day_7 = Carbon::now()
        ->subdays(7)
        ->format('d-m-Y');
    ?>
    ...
    $companies = [];
    $purchases_companies = [];
    $products = \App\Product::select('id', 'name')
        ->where('status', 1)
        ->get();
    if ($set_category != 0 && $set_category != '') {
        $records = Record::select('id', 'supplier_company_id')
            ->where('category_id', $_GET['category'])
            ->where('status', 1)
            ->where('deleted_status', 0)
            ->get()
            ->groupBy('supplier_company_id');
        $purchases = PurchaseRecord::select('id', 'supplier_company_id')
            ->where('category_id', $_GET['category'])
            ->where('status', 1)
            ->where('deleted_status', 0)
            ->get()
...
function sLineInitial() {
    ...
    var url = '{{ route('SlineChartMonthly') }}';
...
success:function(data) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var options = {
                        series: [{
                                name: "Supplier Sales",
                                data: data.supplier_sale
                            },
                            {
                                name: "Supplier Purchases",
                                data: data.supplier_purchase
                            }
                        ],
                        chart: {
                            type: 'bar',
                            ...
                            events: {
                                dataPointSelection: function(event, chartContext, config) {
                                    if (config.seriesIndex == 1)
                                        ...
                                        var series = 'sales';
                                    var company = config.w.config.xaxis.categories[config
                                        .dataPointIndex];
                        ...
                        xaxis: {
                            categories: data.supplier_category,
                        },
                        colors: ['#008ffb', '#FEB019'],
                    }
    ...
    <script>
            ...
            $.ajax({
                ...
                success: function(data) {
                    //console.log(data);
                    $('#TOTAL_DELIVERIES').html(data.TOTAL_DELIVERIES);
                    $('#PENDING_RECORDS').html(data.PENDING_RECORDS);
                    $('#PAID_INVOICES').html(data.PAID_INVOICES);
                    $('#BALANCE_AMOUNT').html(data.BALANCE_AMOUNT);
                    $('#unpaid_invoice').html(data.unpaid_invoice);
                    $('#unpaid_balance').html(data.unpaid_balance);
                    $('#overdue_invoice').html(data.overdue_invoice);
                    $('#overdue_balance').html(data.overdue_balance);
                    $('#tax_withheld').html('$' + data.tax_withheld);
                    $('#gross_earnings').html('$' + data.gross_earnings);
                    $('#net_earnings').html('$' + data.net_earnings);

                    $('#d7_total').html(data.d7_total);
                    $('#date7').html(data.date7);

                    $('#d6_total').html(data.d6_total);
                    $('#date6').html(data.date6);

                    $('#d5_total').html(data.d5_total);
                    $('#date5').html(data.date5);

                    $('#date4').html(data.date4);
                    $('#d4_total').html(data.d4_total);

                    $('#date3').html(data.date3);
                    $('#d3_total').html(data.d3_total);

                    $('#date2').html(data.date2);
                    $('#d2_total').html(data.d2_total);

                    $('#date1').html(data.date1);
                    $('#d1_total').html(data.d1_total);
                    ...
                    output +=       "<div class='mb-4'>" +
                                    "<div class='w-browser-details'>"+
                                        ...
                                        "<div class='w-browser-stats'>"+
                                            "<div class='progress' data-toggle='tooltip' data-placement='top' data-original-title='"+ data.p[i] +"%'>" +
                                                "<div class='progress-bar bg-gradient-primary' role='progressbar' style='width: "+ data.p[i] +"%'    aria-valuenow='"+ data.p[i] +"' aria-valuemin='0' aria-valuemax='100'>"+ data.p[i] + "%" + "</div>"+
                                            "</div>"+
                                        "</div>"+
                                        "<p class='browser-count' id='bro' style='float: right;'>"+ data.v[i] +'L' +"</p>"+
                                    "</div>"+
                                "</div>";
                    }

                    console.log(output);
                    $('#sidechart').append(output);

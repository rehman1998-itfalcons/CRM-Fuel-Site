    	...
    	$("#select-range").change(function () {
          	...
          	if (value != '') {
            	if (value == 'Custom Range') {
                	...
                	$('#custom-range').css('display','none');
                      ...
                      $.post('{{ url('/overdueInvoices-filter') }}',{_token:'{{ csrf_token() }}', type:value}, function(data) {
      	...
      	$('#submit-btn').click(function () {
        	...
        	var to = $("#to_date").val();
          	...
          	if (from != '' && to != '') {
                  	$('#body_result').html('<div class="loader mx-auto"></div>');
              	$.post('{{ url('/overdueInvoices-filter') }}',{_token:'{{ csrf_token() }}', type:value,from:from,to:to}, function(data) {
                	$('#body_result').html(data);
              	});
            }
        });

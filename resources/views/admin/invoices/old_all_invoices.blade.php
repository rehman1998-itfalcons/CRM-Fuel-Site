      ...
      <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
          ...
          <hr>
            <div class="row">
                ...
                <div class="col-md-3">
                <a href="{{ url('upload-invoices') }}"><button  style="float: right;" type="button" class="btn btn-md btn-primary" >Upload Sales invoices</button></a>
                </div>
              	<div class="col-md-3">
                  <a href="{{ url('sale-sync') }}"><button  style="float: right;" type="button" class="btn btn-md btn-success" >Sync Sales invoices UID</button></a>
                  </div>
              </div>
           <br>
            ...
            <table id="html5-extension" class="table table-hover non-hover" style="width:100%">
              ...
              <tbody id="body_result">
                ...
                <tr>
                      ...
                      <td>{{ \Carbon\Carbon::parse($record->datetime)->format('d-m-Y') }}</td>
    	...
    	$("#select-range").change(function () {
          	...
          	if (value != '') {
            	if (value == 'Custom Range') {
                	...
                	$('#custom-range').css('display','none');
                      ...
                      $.post('{{ url('/allInvoices-filter') }}',{_token:'{{ csrf_token() }}', type:value}, function(data) {
      	...
      	$('#submit-btn').click(function () {
        	...
        	var to = $("#to_date").val();
          	...
          	if (from != '' && to != '') {
                  	$('#body_result').html('<div class="loader mx-auto"></div>');
              	$.post('{{ url('/allInvoices-filter') }}',{_token:'{{ csrf_token() }}', type:value,from:from,to:to}, function(data) {
                	$('#body_result').html(data);
              	});
            }
        });

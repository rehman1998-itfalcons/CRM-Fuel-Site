...
@section('contents')
	...
	<?php
		...
		if ($category == 0)
			$records = Record::where('paid_status',1)->where('status',1)->where('deleted_status',0)->get();
		elseif($category != 0)
			$records = Record::where('category_id',$category)->where('paid_status',1)->where('status',1)->where('deleted_status',0)->get();
	?>

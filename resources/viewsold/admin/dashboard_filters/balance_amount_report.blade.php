...
@section('contents')
	...
	<?php
		...
		elseif($category != 0 && $month == '')
			$records = Record::where('category_id',$category)->where('status',1)->where('deleted_status',0)->get();
		elseif($category == 0 && $month != '')
			$records = Record::whereMonth('datetime',$month)->where('status',1)->where('deleted_status',0)->get();
		elseif($category != 0 && $month != '')
			$records = Record::where('category_id',$category)->whereMonth('datetime',$month)->where('status',1)->where('deleted_status',0)->get();
        ...
        <div class="widget-header">
          <div class="row">
            <div class="col-xl-12 col-md-12 col-sm-12 col-12">
              <h4>Balance Amount ($<span class="total"></span>)</h4>
              <hr>
                ...
                <table id="html5-extension" class="table table-hover non-hover" style="width:100%">
                  ...
                  <tbody id="body_result">
                    ...
                    @foreach ($records as $sale)
                    	...
                    	<?php $total = $total + $sale->total_amount; ?>
                    @endforeach
                  </tbody>
                </table>
            </div>

...
class linechart extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:linechart';
    ...
    {
        $cron_job =  DB::table('dashboard_cache')->where('value', 'linechart')->first();
        if ($cron_job->value == 'linechart' && $cron_job->status == 1) {
            ...
            $minutes = 11;
            $output = \Cache::remember('linechart', $minutes, function () use ($Categories_data, $purchases_data, $sale_data) {
            ...
            DB::table('dashboard_cache')->where('value', 'supplierchart')->update([
                'status' => 1
            ]);
        }
    }

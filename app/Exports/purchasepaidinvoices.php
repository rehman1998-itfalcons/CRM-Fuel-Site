...
class purchasepaidinvoices implements FromCollection, WithHeadings
{
    /**
    ...
    public function collection()
    {
        ...
        if ( $this->start_date == null || $this->end_date == null ) {
            ...
            $data = PurchaseRecord::join('categories', 'purchase_records.category_id', '=', 'categories.id')
                ...
                ]) ->where('purchase_records.status', 1)->where('purchase_records.deleted_status', 0)->where('purchase_records.paid_status',1)->where(function($q){
                ...
                ->select([
                    ...
                    DB::raw('categories.name as category_name'),
                    DB::raw('supplier_companies.name as fule_Company_name')
                ]) ->where('purchase_records.status', 1)->where('purchase_records.deleted_status', 0)->where('purchase_records.paid_status',1)
            ->get();
            }
        }

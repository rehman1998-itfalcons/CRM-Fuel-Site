...
use App\Http\thousand_seperator;
...
class RecordsController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return Datatables::eloquent($data)
                ...
                ->addColumn('datetime', function ($data) {
                    ...
                    return Carbon::parse($data->datetime)->format('d-m-Y');
                })
                ->editColumn('total_amount', function ($data) {
                    $amount = '<span >$</span>' . Thousandsep($data->total_amount);
                    return  $amount;
                })
                ...
                ->addColumn('action', function ($data) {
                    $button = '<div class="text-center"> <a href="' . route('records.show', $data->id) . '"><button type="button" name="edit" id="' . $data->id . '" class="btn btn-sm btn-primary">View</button></a></div>';
                    return $button;
                })
                ->orderColumn('datetime', function ($query, $order) {
                    $query->orderBy('datetime', $order);
                })
                ->orderColumn('subCompany_name', function ($query, $order) {
                    $query->orderBy('subCompany_name', $order);
                })
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new recordexport($search, $start_date, $end_date), 'Records.xlsx');
    }
        ...
        foreach ($item_list as $items) {
            if (is_array($items) || is_object($items)) {
                ...
                foreach ($items as $item) {
                    if (isset($item['Number']) && isset($record->invoice_number) && $record->invoice_number == $item['Number']) {

                        $myob->FileGetContents(company_uri() . 'Sale/Invoice/Item/' . $item['UID'], 'delete', '', $token);

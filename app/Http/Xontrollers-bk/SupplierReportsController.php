  	...
  	public function supplierReportGeneration(Request $request)
    {
        ...
        $to = Carbon::parse($request->to)->format('Y-m-d');        
            ...
            if ($request->company_id == 'all_companies') {
                ...
                $records = Record::select('id','supplier_company_id','status','datetime')->whereBetween('datetime', [$from, $to])->where('category_id',$request->category_id)->where('sub_company_id',$request->sub_company_id)->where('status',1)->where('deleted_status',0)->get()->groupBy('supplier_company_id');
                }

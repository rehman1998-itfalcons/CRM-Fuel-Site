...
class ManualMassmatchController extends Controller
{
    public function addManualMassMatch(){
      $records = Record::select('id','invoice_number')->where('status',1)->where('deleted_status',0)->where('mass_match_status', 0)->get();
      $purchase_records = PurchaseRecord::select('id','invoice_number')->where('match_status',0)->where('status',1)->get();
